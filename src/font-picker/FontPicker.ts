...
export default class FontPicker {
  ...
  private activeFont?: Font;
	...
	private generateUI(sort: SortOption): void {
        ...
        const combinedFonts = fonts.concat(this.localFonts);
				if (sort === "alphabet") {
					combinedFonts.sort((font1: Font, font2: Font): number =>
						font1.family.localeCompare(font2.family),
					);
				}
				this.generateFontList(combinedFonts);
    ...
    if (this.ul) {
      this.fontPickerDiv?.removeChild(this.ul);
      this.ul = undefined;
    }
    ...
    const isLocalFont = !this.fontManager.getFonts().get(font.family);
    let fontId;
    if(!isLocalFont) {
      fontId = getFontId(font.family);
    } else {
      fontId = font.id;
    }
    ...
    if (isLocalFont) {
      fontButton.style.fontFamily = font.family;
    }
	...
	/**
	 * Add font to font picker
	 */
	public addLocalFont(fontFamily: string, index?: number): void {
    const font: Font = {
      family: fontFamily,
      id: fontFamily.replaceAll(" ", "-").toLocaleLowerCase(),
      category: "display",
      scripts: [],
      variants: []
    };
    if (!this.checkIfFontExists(fontFamily)) {
      this.localFonts.push(font);
  ...
  public addLocalFonts(fontFamilys: string[], sortAfter?: boolean) {
    for (const fontFamily of fontFamilys) {
      const font: Font = {
        family: fontFamily,
        id: fontFamily.replaceAll(" ", "-").toLocaleLowerCase(),
        category: "display",
    ...
    const googleFonts = Array.from(this.fontManager.getFonts().values());
    const combinedFonts = this.localFonts.concat(googleFonts);

    if (sortAfter && this.sort === "alphabet") {
      combinedFonts.sort((font1: Font, font2: Font): number =>
        font1.family.localeCompare(font2.family),
      );
    }
  ...
  private checkIfFontExists(fontFamily: string) {
		const isGoogleFont = !!this.fontManager.getFonts().get(fontFamily);
    const isLocalFont = !!this.localFonts.find(font => font.family === fontFamily);
    return isGoogleFont || isLocalFont;
  }
	...
	public getActiveFont(): Font {
		return this.activeFont ? this.activeFont : this.fontManager.getActiveFont();
	}
    ...
    const isLocalFont = !this.fontManager.getFonts().get(fontFamily);
    let localFont;
    let fontId;
    if(!isLocalFont) {
      this.fontManager.setActiveFont(fontFamily);
      this.activeFont = undefined;
      fontId = getFontId(fontFamily);
    } else {
      localFont = this.localFonts.find(font => font.family === fontFamily);
      this.activeFont = localFont;
      fontId = localFont ? localFont.id : undefined;
    }

		// Write new font family in dropdown button and highlight font entry in the list
    if (this.dropdownFamily) {
    ...
    if (isLocalFont && localFont && this.onChange) {
      this.onChange(localFont);
    }
	}
